...
if not "%skip-stage0%" == "true" (
  ...
  "%tar_tool%" -xzf libxml2-v%libxml_version%.tar.gz --exclude "test/*" || exit /b 1
  ...
  "%tar_tool%" -xzf zlib-%zlib_version%.tar.gz || exit /b 1
  ...
  "%tar_tool%" -xzf zstd-%zstd_version%.tar.gz --exclude "tests/*" || exit /b 1
)
...
cd build32_stage0
if "%skip-stage0%" == "true" (
  call :use_staged_deps "%build_dir%\build32_stage0" || exit /b 1
) else (
...
if "%skip-stage0%" == "true" (
  call :use_staged_deps "%build_dir%\build_%arch%_stage0" || exit /b 1
) else (
  call :do_build_libxml || exit /b 1
  call :do_build_zlib || exit /b 1
  call :do_build_zstd || exit /b 1
...
::==============================================================================
:: Use dependencies staged by a previous stage0 build.
::==============================================================================
:use_staged_deps
set "libxmldir=%~1\libxmlbuild\install"
set "zlibdir=%~1\zlibbuild\install"
set "zstddir=%~1\zstdbuild\install"
if not exist "%libxmldir%\lib\libxml2s.lib" (
  echo Stage0 libxml install is missing: %libxmldir%
  echo Run the stage0 build first to create the staged dependencies.
  exit /b 1
)
if not exist "%zlibdir%\lib\zs.lib" (
  echo Stage0 zlib install is missing: %zlibdir%
  echo Run the stage0 build first to create the staged dependencies.
  exit /b 1
)
if not exist "%zstddir%\lib\zstd_static.lib" (
  echo Stage0 zstd install is missing: %zstddir%
  echo Run the stage0 build first to create the staged dependencies.
  exit /b 1
)
