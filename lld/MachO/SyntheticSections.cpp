...
void ObjCStubsSection::addEntry(Symbol *sym) {
  ...
  if (isObjCClassStubSymbol(sym)) {
    ...
    if (auto *dysym = dyn_cast<DylibSymbol>(classSym)) {
      // A class the object imports weakly stays a weak import: the stub then
      // messages nil where the class is unavailable. The dylib's own state may
      // already have been gathered from its symbols when the dylib precedes
      // the object with the stub.
    ...
    if (needsBinding(classSym))
      in.got->addEntry(classSym);
    classSym->used = true;
  } else {
