...
public:
  ...
  bool OutputColoredStringTruncated(int right_pad, StringRef string,
                                    ...
                                    bool use_grey_background) {
    auto start_index = (use_blue_background ? BlackOnRed : use_grey_background ? BlackOnGrey : BlackOnBlack);
    ...
    while (!string.empty()) {
      ...
      } else if (values[0] >= ANSI_FG_COLOR_BLACK && values[0] <= ANSI_FG_COLOR_WHITE) {
        auto color_index = start_index + values[0] - ANSI_FG_COLOR_BLACK;

        ::wcolor_set(m_window, color_index, nullptr);
      } else if (values[0] >= ANSI_FG_COLOR_BRIGHT_BLACK && values[0] <= ANSI_FG_COLOR_BRIGHT_WHITE) {
        auto color_index = start_index + 8 + values[0] - ANSI_FG_COLOR_BRIGHT_BLACK;
...
public:
  ...
  std::string UntabifyLine(StringRef line) {
    ...
    // Note: Due to the complexities of utf8 rules this will only take prefixed tabs into account
        ...
        if (m_sc.line_entry.IsValid()) {
          ...
          if (m_file_sp && m_file_sp->GetSupportFile()->GetSpecOnly() ==
            ...
            if (m_file_sp) {
              ...
              // Align on tab size
              m_line_width = (((m_line_width + 5) + m_tab_size - 1) / m_tab_size) * m_tab_size - 5;
      ...
      for (size_t i = 0; i < num_visible_lines; ++i) {
        ...
        if (curr_line < num_source_lines) {
          ...
          bool wasWritten = window.OutputColoredStringTruncated(
              1, line, m_first_visible_column, is_pc_line, line_is_selected && !is_pc_line);
          if ((!wasWritten && (line_is_selected || is_pc_line)) || (line_is_selected && is_pc_line)) {
            // Draw an empty space to show the selected/PC line if empty,
            // or draw '<' if nothing is visible because of scrolling too much
            // to the right.
            window.ColorSet(line_is_selected && is_pc_line ? WhiteOnGrey : is_pc_line ? WhiteOnRed : WhiteOnGrey);
            window.PutCStringTruncated(
...
void IOHandlerCursesGUI::Activate() {
  ...
  if (!m_app_ap) {
    ...
    // All colors with black an blue background.
    for (int i = 0; i < 256; ++i) {
