...
auto fg_GetPythonLibraryCandidates() -> std::vector<std::string> {
  ...
  const char *script =
      ...
      "        print(path)\n"
      "if framework:\n"
      "    for prefix_name in ('base', 'platbase', 'installed_base',\n"
      "                        'installed_platbase', 'prefix', 'exec_prefix'):\n"
      "        prefix = sysconfig.get_config_var(prefix_name)\n"
      "        if not prefix:\n"
      "            continue\n"
      "        path = os.path.join(prefix, framework)\n"
      "        if os.path.exists(path):\n"
      "            print(path)\n"
      "seen = set()\n"
...
DForwardPythonFunction(int, PyErr_GivenExceptionMatches,
                       ...
                       (given, exception))
DForwardPythonFunction(PyObject *, PyErr_NoMemory, (), ())
...
DForwardPythonFunction(long long, PyLong_AsLongLong, (PyObject *object),
                       (object))
DForwardPythonFunction(size_t, PyLong_AsSize_t, (PyObject *object), (object))
