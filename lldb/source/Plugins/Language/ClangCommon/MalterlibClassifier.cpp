...
public:
    ...
    void f_PopulateStyle(EClassification _Classification, uint32_t _Color) {
      ...
      m_ColorStyles[_Classification] = HighlightStyle::ColorStyle(true, llvm::formatv("\x1B[38;5;{0}m", Color256), "\x1B[0m");
    }
    ...
    EClassification f_Classify(std::string _Identifier) {
        ...
        ssize_t Length = _Identifier.length();
        if (Length < 3)
            return EClassification::EClassification_Unknown;
        for (ssize_t i = ssize_t(gc_MaxPrefixLen); i >= 0; --i)
        {
            if (i > Length)
                continue;
