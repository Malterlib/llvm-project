...
determineClangStyle(const ClangHighlighter &highlighter,
  ...
  if (tok::isStringLiteral(token.getKind()) || (in_global_include && token.getKind() != tok::greater))
...
HighlightStyle::ColorStyle ClangHighlighter::highlightIdentifier(llvm::StringRef identifier,
  HighlightStyle::ColorStyle default_style) const {
  return malterlibClassifier.highlightIdentifier(identifier, default_style);
}

HighlightStyle::ColorStyle ClangHighlighter::highlightNumber(llvm::StringRef identifier,
  HighlightStyle::ColorStyle default_style) const {
  return malterlibClassifier.highlightNumber(identifier, default_style);
}

HighlightStyle::ColorStyle ClangHighlighter::highlightString(llvm::StringRef identifier,
  HighlightStyle::ColorStyle default_style) const {
  return malterlibClassifier.highlightString(identifier, default_style);
}

HighlightStyle::ColorStyle ClangHighlighter::highlightPunctuation(llvm::StringRef identifier,
  HighlightStyle::ColorStyle default_style) const {
  return malterlibClassifier.highlightPunctuation(identifier, default_style);
}

HighlightStyle::ColorStyle ClangHighlighter::highlightKeyword(llvm::StringRef identifier,
  HighlightStyle::ColorStyle default_style) const {
  return malterlibClassifier.highlightKeyword(identifier, default_style);
}

HighlightStyle::ColorStyle ClangHighlighter::highlightComment(llvm::StringRef identifier,
  HighlightStyle::ColorStyle default_style) const {
...
void ClangHighlighter::Highlight(const HighlightStyle &options,
  ...
  while (!exit) {
    ...
    HighlightStyle::ColorStyle color =
        determineClangStyle(*this, token, tok_str, options, in_pp_directive, was_in_pp_directive, in_global_include);
