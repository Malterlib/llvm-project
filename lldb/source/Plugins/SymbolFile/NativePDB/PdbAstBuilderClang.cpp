...
static clang::QualType GetClangTypeForPrimitiveTemplateArgument(
    PdbAstBuilderClang &builder, llvm::ms_demangle::PrimitiveKind kind) {
...
static void ApplyTypeDeclarationMetadata(PdbAstBuilderClang &builder,
                                         clang::QualType qt,
                                         const Declaration &declaration) {
...
static void ApplyTemplateArgumentDeclarationMetadataFromName(
    PdbAstBuilderClang &builder, clang::QualType qt, llvm::StringRef name) {
  if (qt.isNull())
    return;
...
static clang::QualType CreateFallbackTemplateSpecializationTypeFromName(
    PdbAstBuilderClang &builder, llvm::StringRef name,
...
static clang::QualType
FindPdbTemplateArgumentType(PdbAstBuilderClang &builder, llvm::StringRef name,
...
static CompilerType CreateClassTemplateSpecializationType(
    PdbAstBuilderClang &builder, clang::DeclContext *context,
    llvm::StringRef uname, clang::TagTypeKind ttk,
    TypeSystemClang::TemplateParameterInfos const &template_param_infos,
    ClangASTMetadata const &metadata, bool require_new);

static clang::QualType
GetClangTypeForTemplateArgument(PdbAstBuilderClang &builder,
                                llvm::ms_demangle::Node &node,
                                TypeIndex current_type_index);

static clang::QualType
GetClangTypeForMSDemangleType(PdbAstBuilderClang &builder,
                              llvm::ms_demangle::Node &node,
                              TypeIndex current_type_index);
...
static clang::QualType CreateFallbackTemplateArgumentRecordType(
  ...
  CompilerType ct = clang_ast.CreateRecordType(
      context, OptionalClangModuleID(), name,
...
static bool InsertTemplateArgumentFromMSDemangleNode(
    PdbAstBuilderClang &builder, llvm::ms_demangle::Node &node,
...
static bool ParseTemplateParameterInfosFromNodes(
    PdbAstBuilderClang &builder, llvm::ms_demangle::NodeArrayNode &params,
...
static clang::DeclContext *CreateFallbackTemplateArgumentDeclContext(
    PdbAstBuilderClang &builder, llvm::ms_demangle::QualifiedNameNode &qn) {
  TypeSystemClang &clang_ast = builder.clang();
  clang::DeclContext *context = clang_ast.GetTranslationUnitDecl();
...
GetClangTypeForMSDemangleFunctionSignature(
    PdbAstBuilderClang &builder, llvm::ms_demangle::FunctionSignatureNode &signature,
    TypeIndex current_type_index) {
...
static clang::QualType
GetClangTypeForTemplateArgumentName(PdbAstBuilderClang &builder,
                                    llvm::StringRef name) {
  TemplateArgumentTypeName unqualified = StripTopLevelQualifiers(name);
...
static bool ParseTemplateParameterInfosFromName(
    PdbAstBuilderClang &builder, llvm::StringRef name, TypeIndex current_type_index,
    TypeSystemClang::TemplateParameterInfos &template_param_infos) {
...
CreateNamespaceDeclContextForQualifiedName(PdbAstBuilderClang &builder,
...
static clang::QualType CreateFallbackTemplateArgumentSpecializationType(
  ...
  if (std::optional<Declaration> declaration =
    ...
    metadata.SetDeclaration(*declaration);
  CompilerType ct = CreateClassTemplateSpecializationType(
      builder, context, GetUnqualifiedTypeName(name), ttk,
...
static clang::QualType CreateFallbackTemplateSpecializationTypeFromName(
  ...
  CompilerType ct = CreateClassTemplateSpecializationType(
      builder, context, GetUnqualifiedTypeName(type_name),
...
static bool ParseTemplateParameterInfos(
    PdbAstBuilderClang &builder, llvm::codeview::TagRecord const &record,
    TypeIndex current_type_index,
    TypeSystemClang::TemplateParameterInfos &template_param_infos) {
...
static CompilerType CreateClassTemplateSpecializationType(
  ...
  clang::ClassTemplateDecl *class_template_decl =
      clang_ast.ParseClassTemplateDecl(
          context, OptionalClangModuleID(), uname.str().c_str(),
          llvm::to_underlying(ttk), template_param_infos);
...
void PdbAstBuilderClang::RegisterTagType(PdbTypeSymId type, clang::QualType qt,
                                    bool resolved) {
  if (qt.isNull())
    return;
...
clang::QualType PdbAstBuilderClang::CreateRecordType(PdbTypeSymId id,
  ...
  if (!FindLastTopLevelScopeSeparator(template_uname) &&
    ...
    ct = CreateClassTemplateSpecializationType(
        *this, template_context, template_uname, ttk,
        template_param_infos, metadata, false);
  }
  ...
  if (!ct.IsValid()) {
    ct = m_clang.CreateRecordType(context, OptionalClangModuleID(),
